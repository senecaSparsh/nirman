...
export async function approvePurchaseOrder(poId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    return tx.purchaseOrder.update({ where: { id: poId }, data: { status: "APPROVED" } });
  });
}
...
export async function cancelPurchaseOrder(poId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    return tx.purchaseOrder.update({ where: { id: poId }, data: { status: "CANCELLED" } });
  });
}
...
export async function receiveGoods(input: ReceiveGoodsInput) {
  return withStockTransaction(async (tx) => {
    ...
    // 5. Update Material.currentCost (global average of all location MACs)
    for (const line of input.lines) {
      const avgMac = await tx.stockLocationItem.aggregate({
        where: { materialId: line.materialId, location: { deletedAt: null } },
        _avg: { movingAvgCost: true },
      });
      if (avgMac._avg.movingAvgCost) {
        await tx.material.update({
          where: { id: line.materialId },
          data: { currentCost: avgMac._avg.movingAvgCost },
        });
      }
    }
