...
export const PATCH = apiHandler(async (req: NextRequest, ctx: { params: Promise<{ id: string; cid: string }> }) => {
  ...
  try {
    ...
    return json({ error: (err instanceof Error ? err.message : "Failed to update cost component") }, { status: 400 });
  }
});
...
export const DELETE = apiHandler(async (_req: NextRequest, ctx: { params: Promise<{ id: string; cid: string }> }) => {
  ...
  try {
    await deleteLandCostComponent(cid, user.id);
    revalidatePath("/land");
    revalidatePath(`/land/${id}`);
    revalidatePath("/m/land");
    revalidatePath("/m/real-estate?tab=land");
    ...
    return json({ error: (err instanceof Error ? err.message : "Failed to delete cost component") }, { status: 400 });
  }
});
