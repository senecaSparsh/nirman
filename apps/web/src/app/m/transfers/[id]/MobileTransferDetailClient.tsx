...
export function MobileTransferDetailClient({
  ...
  const destCompanyMembership = transfer.userMemberships.find(
    (m) => m.id === transfer.toCompanyId,
  );
  const sourceCompanyMembership = transfer.userMemberships.find(
    (m) => m.id === transfer.fromCompanyId,
  );
  ...
  const handleReturnToSource = async () => {
    ...
    try {
      ...
      haptic([10, 30, 10]);
      toast.error(
        err instanceof Error ? err.message : "Failed to return to source",
      );
    } finally {
      setActing(null);
      setShowReturn(false);
    ...
    <div className="pb-20">
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2">
          <TransportField
            ...
            value={
              transfer.deliveryMode
                ? transfer.deliveryMode.replace(/_/g, " ").toLowerCase()
                : "Not specified"
            }
            muted={!transfer.deliveryMode}
          />
          <TransportField
            icon={Truck}
            label="Vehicle type"
            value={
              transfer.vehicleType
                ? transfer.vehicleType.replace(/_/g, " ").toLowerCase()
                : "Not specified"
            }
            muted={!transfer.vehicleType}
          />
          <TransportField
            icon={Hash}
            label="Vehicle no"
            value={transfer.vehicleNumber ?? "Not specified"}
            mono={!!transfer.vehicleNumber}
            muted={!transfer.vehicleNumber}
          />
          <TransportField
            icon={FileText}
            label="Challan no"
          ...
          <TransportField
            icon={Building2}
            label="Transporter"
            value={transfer.transporterName ?? "Not specified"}
            muted={!transfer.transporterName}
          />
          <TransportField
            icon={Boxes}
            label="Packages"
            value={
              transfer.packageCount != null
                ? String(transfer.packageCount)
                : "Not specified"
            }
            muted={transfer.packageCount == null}
          />
          <TransportField
            icon={User}
            label="Driver"
            value={transfer.driverName ?? "Not specified"}
            muted={!transfer.driverName}
          />
          <TransportField
            icon={Phone}
            label="Driver phone"
            value={transfer.driverPhone ?? "Not specified"}
            mono={!!transfer.driverPhone}
            muted={!transfer.driverPhone}
          />
        </div>
      </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2">
          <TransportField
            icon={Hash}
            label="Ticket no"
            value={transfer.weighbridgeTicketNo ?? "Not specified"}
            mono={!!transfer.weighbridgeTicketNo}
            muted={!transfer.weighbridgeTicketNo}
          />
          <TransportField
            icon={Weight}
            label="Gross wt"
            value={
              transfer.grossWeight != null
                ? `${formatNumber(transfer.grossWeight, 3)} kg`
                : "Not specified"
            }
            muted={transfer.grossWeight == null}
          />
          <TransportField
            icon={Weight}
            label="Tare wt"
            value={
              transfer.tareWeight != null
                ? `${formatNumber(transfer.tareWeight, 3)} kg`
                : "Not specified"
            }
            muted={transfer.tareWeight == null}
          />
          <TransportField
            icon={Weight}
            label="Net wt"
            value={
              transfer.netWeight != null
                ? `${formatNumber(transfer.netWeight, 3)} kg`
                : "Not specified"
            }
            muted={transfer.netWeight == null}
          />
        </div>
      </div>
      ...
      <div
        ...
        style={{
          ...
          backgroundColor:
            transfer.geoFenceOk === false
              ? "color-mix(in srgb, var(--color-stop) 4%, var(--color-paper))"
              : transfer.geoFenceOk === true
                ? "color-mix(in srgb, var(--color-go) 4%, var(--color-paper))"
                : "var(--color-paper)",
        }}
      >
        <div className="flex items-center gap-1.5 mb-2">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2">
          <TransportField
            ...
            muted={transfer.geoFenceOk == null}
          />
          <TransportField
            icon={MapPin}
            label="Distance"
            value={
              transfer.geoFenceDistance != null
                ? `${formatNumber(transfer.geoFenceDistance, 0)}m from center`
                : "Not specified"
            }
            muted={transfer.geoFenceDistance == null}
          />
          <TransportField
            icon={Navigation}
            label="GPS"
            value={
              transfer.receiverLat != null && transfer.receiverLng != null
                ? `${transfer.receiverLat.toFixed(4)}, ${transfer.receiverLng.toFixed(4)}`
                : "Not captured"
            }
            mono={transfer.receiverLat != null}
            muted={transfer.receiverLat == null}
          />
          <TransportField
            icon={MapPin}
            label="Location"
            value={transfer.receiverLocation ?? "Not specified"}
            muted={!transfer.receiverLocation}
          />
        </div>
      </div>
              ...
              details={
                [
                  transfer.shortageRemarks
                    ? `Shortage: ${transfer.shortageRemarks}`
                    : null,
                  transfer.damageRemarks
                    ? `Damage: ${transfer.damageRemarks}`
                    : null,
          ...
          {transfer.lines.map((l, idx) => {
            ...
            return (
              <Link
                ...
                className="block rounded-[0.5rem] border p-2.5 active:opacity-80 transition-opacity"
                style={{
                  borderColor: hasShortage
                    ? "color-mix(in srgb, var(--color-stop) 30%, var(--color-line))"
                    : "var(--color-line)",
                  backgroundColor: "var(--color-paper)",
                }}
              >
                ...
                {l.unitCostAtSource != null || l.unitTransferPrice != null ? (
                  <div
                    className="flex items-center gap-3 mt-1.5 pl-5 pt-1.5 border-t"
                    style={{ borderColor: "var(--color-line)" }}
                  >
      ...
      {transfer.lines.length > 0
        ? (() => {
            const totalSourceValue = transfer.lines.reduce(
              (s, l) =>
                s +
                (l.unitCostAtSource != null ? l.qty * l.unitCostAtSource : 0),
              0,
            );
            const totalTransferValue = transfer.lines.reduce(
              (s, l) => s + (l.lineTransferTotal ?? 0),
              0,
            );
            const hasAnyCost = transfer.lines.some(
              (l) => l.unitCostAtSource != null || l.lineTransferTotal != null,
            );
            ...
            return (
              ...
              >
                <div className="space-y-1">
                  ...
                  totalTransferValue !== totalSourceValue ? (
                    <div
                      className="flex items-center justify-between pt-1 border-t"
                      style={{ borderColor: "var(--color-line)" }}
                    >
      ...
      {canManage && isInterCompany && !isCompleted && !isCancelled
        ? (() => {
            ...
            const needsSwitchToDest =
              isSourceCompany &&
              !isDestCompany &&
              isInTransit &&
              destCompanyMembership;
            // User is on dest company but needs to be on source company to dispatch
            const needsSwitchToSource =
              isDestCompany &&
              !isSourceCompany &&
              isDraft &&
              sourceCompanyMembership;
            if (!needsSwitchToDest && !needsSwitchToSource) return null;
            const targetCompany = needsSwitchToDest
              ? destCompanyMembership!
              : sourceCompanyMembership!;
            const targetLabel = needsSwitchToDest ? "Receive" : "Dispatch";
            return (
              <div
                className="rounded-[0.5rem] border-2 px-3 py-2.5 mb-3 flex items-center gap-2"
                style={{
                  borderColor: "var(--color-signal)",
                  backgroundColor:
                    "color-mix(in srgb, var(--color-signal) 8%, var(--color-paper))",
                }}
              >
                <LogIn
                  className="size-4 shrink-0"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    You are logged into{" "}
                    {transfer.isSourceCompany
                      ? transfer.fromLocation.companyName
                      : transfer.toLocation.companyName}
                    . The {targetLabel.toLowerCase()} must be done from{" "}
                    {targetCompany.name}.
                  </p>
                </div>
                <button
        ...
        >
          ...
          >
            ...
            <span className="font-mono font-semibold">
              {transfer.gatePass.gatePassNumber}
            </span>{" "}
            —{" "}
            {transfer.gatePass.status === "PENDING"
              ? "awaiting approval. Dispatch blocked until approved."
              : transfer.gatePass.status === "APPROVED"
                ? "approved — ready to dispatch."
                : transfer.gatePass.status === "EXITED"
                  ? "items have exited."
                  : transfer.gatePass.status === "REJECTED"
            ...
            {isDraft ? (
              canDispatch ? (
                ...
                >
                  ...
                  Waiting for{" "}
                  {transfer.fromLocation.companyName ?? "source company"} to
                  dispatch
                </div>
              )
            ) : isInTransit ? (
              canReceive ? (
                ...
                >
                  ...
                  In transit — waiting for{" "}
                  {transfer.toLocation.companyName ?? "destination"} to receive
                </div>
              )
            ) : null}
          </div>
          ...
          <div className="flex gap-1 mt-1.5 flex-wrap">
            {photos.map((p, i) => (
              ...
              <img
                ...
                className="size-12 rounded-[0.25rem] object-cover"
        ...
        {signature ? (
          <div className="mt-1.5">
            ...
            <img
              ...
              className="max-h-12 rounded-[0.25rem]"
              style={{ border: "1px solid var(--color-line)" }}
            />
          </div>
        ) : null}
  ...
  return (
    <div className="flex items-center gap-1.5 min-w-0">
      <Icon
        className="size-3 shrink-0"
        style={{
          color: muted ? "var(--color-ink-300)" : "var(--color-ink-400)",
        }}
      />
