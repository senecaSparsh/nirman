...
export function MobileSaleDetailClient({
  ...
  async function handlePayCommission() {
    ...
    try {
      const res = await fetch(`/api/sales/${saleId}/pay-commission`, { method: "POST" });
      const data = await res.json();
  ...
  return (
    <div className="pb-20">
        ...
        >
          {customer ? (
            <Link
              ...
              className="flex items-center gap-1 px-2 py-1.5 text-m-body press"
            >
              <span className="text-m-caption font-semibold uppercase shrink-0" style={{ color: "var(--color-ink-500)" }}>
                Customer
              </span>
              <span className="text-m-caption font-bold ml-auto truncate" style={{ color: "var(--color-ink-950)" }}>
                {customer.name}
              </span>
            </Link>
          ) : null}
          ...
          <div className="flex items-center gap-1 px-2 py-1.5" style={{ borderTop: "1px solid var(--color-line)" }}>
            ...
            <span className="text-m-caption font-bold ml-auto tabular-nums" style={{ color: "var(--color-ink-950)" }}>
              {formatDate(saleDate)}
            </span>
          </div>
          ...
          {notes ? (
            <div className="px-2 py-1.5" style={{ borderTop: "1px solid var(--color-line)" }}>
              ...
              <p className="text-m-caption" style={{ color: "var(--color-ink-700)" }}>{notes}</p>
            </div>
          ) : null}
          ...
          {draftNotes ? (
            <div className="px-2 py-1.5" style={{ borderTop: "1px solid var(--color-line)" }}>
              ...
              <p className="text-m-caption whitespace-pre-wrap" style={{ color: "var(--color-ink-700)" }}>
                {draftNotes}
              </p>
              {draftDocumentUrl ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-m-caption font-semibold mt-1"
            ...
            >
              {dealSource && (
                <div className="flex items-center justify-between px-2 py-1.5" style={{ borderBottom: (brokerName || commissionAmount || dealMaturityMonths || paymentCycle) ? `1px solid var(--color-line)` : undefined }}>
                  <span className="text-m-caption font-medium" style={{ color: "var(--color-ink-500)" }}>Source</span>
              ...
              {brokerName && (
                <div className="flex items-center justify-between px-2 py-1.5" style={{ borderBottom: (commissionAmount || dealMaturityMonths || paymentCycle) ? `1px solid var(--color-line)` : undefined }}>
                  <span className="text-m-caption font-medium" style={{ color: "var(--color-ink-500)" }}>Broker</span>
                  <div className="text-right">
                    <div className="text-m-caption font-bold" style={{ color: "var(--color-ink-950)" }}>{brokerName}</div>
                    {brokerPhone && <div className="text-m-caption" style={{ color: "var(--color-ink-500)" }}>{brokerPhone}</div>}
                  </div>
                </div>
              )}
              {commissionAmount != null && commissionAmount > 0 && (
                <div className="flex items-center justify-between px-2 py-1.5" style={{ borderBottom: (dealMaturityMonths || paymentCycle) ? `1px solid var(--color-line)` : undefined }}>
                  <span className="text-m-caption font-medium" style={{ color: "var(--color-ink-500)" }}>Commission</span>
                  <div className="text-right">
                    <div className="text-m-caption font-bold tabular-nums" style={{ color: "var(--color-ink-950)" }}>{formatCurrency(commissionAmount)}</div>
                    <div className="text-m-caption" style={{ color: commissionStatus === "PAID" ? "var(--color-go)" : "var(--color-ink-500)" }}>
                      {commissionStatus === "PAID" ? "Paid" : commissionStatus === "ACCRUED" ? "Accrued" : "Pending"}
                    </div>
                  </div>
                </div>
              )}
              {canManage && commissionAmount != null && commissionAmount > 0 && commissionStatus !== "PAID" && !isCancelled ? (
                <button
                  onClick={() => { haptic(10); setShowCommission(true); }}
                  className="flex items-center justify-center gap-1.5 w-full px-2 py-1.5 text-m-caption font-bold press"
                  style={{ borderTop: `1px solid var(--color-line)`, color: "var(--color-go)" }}
                >
                  <HandCoins className="size-3" />
                  <span>Pay Broker Commission</span>
                </button>
              ) : null}
              {dealMaturityMonths && (
                <div className="flex items-center justify-between px-2 py-1.5" style={{ borderBottom: paymentCycle ? `1px solid var(--color-line)` : undefined }}>
                  <span className="text-m-caption font-medium" style={{ color: "var(--color-ink-500)" }}>Maturity</span>
                  <span className="text-m-caption font-bold" style={{ color: "var(--color-ink-950)" }}>{dealMaturityMonths} months</span>
                </div>
              )}
              {paymentCycle && (
                <div className="flex items-center justify-between px-2 py-1.5">
                  <span className="text-m-caption font-medium" style={{ color: "var(--color-ink-500)" }}>Payment Cycle</span>
                  <span className="text-m-caption font-bold" style={{ color: "var(--color-ink-950)" }}>{paymentCycle}</span>
                </div>
              )}
            </div>
          )}
      ...
      {(expenses.length > 0 || terms.length > 0) && (
        <div className="mb-2">
          <p className="text-m-caption font-bold uppercase tracking-wide mb-1 px-0.5" style={{ color: "var(--color-steel)" }}>
            Expenses & Terms
          </p>
          <div className="rounded-[0.5rem] border overflow-hidden" style={{ borderColor: "var(--color-line)", backgroundColor: "var(--color-paper)" }}>
            {expenses.map((e, i) => (
              <div key={e.id} className="flex items-center justify-between px-2.5 py-1.5" style={i > 0 || (i === 0 && terms.length > 0) ? { borderTop: "1px solid var(--color-line)" } : undefined}>
                ...
                <span className="text-m-body font-bold tabular-nums" style={{ color: "var(--color-ink-950)" }}>{formatCurrency(e.amount)}</span>
              </div>
            ))}
            {terms.map((t, i) => (
              <div key={t.id} className="px-2.5 py-1.5" style={i > 0 || (i === 0 && expenses.length > 0) ? { borderTop: "1px solid var(--color-line)" } : undefined}>
                <div className="text-m-body font-medium" style={{ color: "var(--color-ink-950)" }}>{t.description}</div>
                <div className="text-m-caption" style={{ color: "var(--color-ink-500)" }}>
      ...
      <div className="mb-3">
        ...
        >
          ...
          <div className="px-2.5 py-2" style={{ borderBottom: "1px solid var(--color-line)" }}>
            ...
            {atsDocumentUrl ? (
              <a href={atsDocumentUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-m-caption text-m-body press" style={{ color: "var(--color-ink-600)" }}>
          ...
          <div className="px-2.5 py-2" style={{ borderBottom: "1px solid var(--color-line)" }}>
            ...
            {bbaDocumentUrl ? (
              <a href={bbaDocumentUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-m-caption text-m-body press" style={{ color: "var(--color-ink-600)" }}>
                <ExternalLink className="size-2.5" />
          ...
          <div className="px-2.5 py-2" style={{ borderBottom: "1px solid var(--color-line)" }}>
            ...
            {registryDocumentUrl ? (
              <a href={registryDocumentUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-m-caption text-m-body press" style={{ color: "var(--color-ink-600)" }}>
                <ExternalLink className="size-2.5" />
          ...
          <div className="px-2.5 py-2">
            ...
            {allotmentDocumentUrl ? (
              <a href={allotmentDocumentUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-m-caption text-m-body press" style={{ color: "var(--color-ink-600)" }}>
                <ExternalLink className="size-2.5" />
                <span className="truncate">{allotmentDocumentName || "View Allotment"}</span>
              </a>
            ) : canManage && !isCancelled ? (
              <MobileDocUploader
                url=""
                label="Upload Allotment"
                onUpload={(url, name) => uploadSaleDocument("ALLOTMENT", url, name)}
              />
            ) : (
              <p className="text-m-caption" style={{ color: "var(--color-ink-400)" }}>Not uploaded</p>
            )}
        ...
        {docUploading && <p className="text-m-caption mt-1" style={{ color: "var(--color-ink-500)" }}>Uploading…</p>}
      </div>
      ...
      <ActionBar>
        ...
        {canManage && !isCancelled ? (
          <button
            onClick={() => setShowEdit(true)}
            className="flex items-center justify-center gap-1.5 w-full rounded-[0.5rem] border py-2 mb-2 text-m-body press"
            style={{ borderColor: "var(--color-line)", backgroundColor: "var(--color-paper)", color: "var(--color-ink-950)" }}
          >
            <Pencil className="size-3.5" />
            <span className="text-m-body font-bold">Edit Sale</span>
          </button>
        ) : null}
      ...
      {showEdit ? (
        <EditSaleModal
          ...
          initialBrokerPhone={brokerPhone}
          initialBrokerAgency={brokerAgency}
          initialCommissionAmount={commissionAmount}
          initialExpectedRegistryDate={expectedRegistryDate}
        ...
        <Modal onClose={() => setShowDeposit(false)} title="Record Deposit">
          <form onSubmit={handleDeposit} className="flex flex-col gap-3">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full rounded-[0.375rem] py-2.5 text-m-body font-bold text-m-body press disabled:opacity-50"
        ...
        <Modal onClose={() => setShowComplete(false)} title="Complete Sale">
          <form onSubmit={handleComplete} className="flex flex-col gap-3">
            ...
            <div className="rounded-[0.625rem] border p-3 flex flex-col gap-3" style={{ borderColor: "color-mix(in srgb, var(--color-signal) 30%, var(--color-line))", backgroundColor: "color-mix(in srgb, var(--color-signal) 4%, var(--color-paper))" }}>
              ...
              <div>
                <label className="text-m-caption font-semibold uppercase block mb-0.5" style={{ color: "var(--color-signal)" }}>
      ...
      {showCommission ? (
        <Modal onClose={() => setShowCommission(false)} title="Pay Broker Commission?">
          <p className="text-m-body mb-3" style={{ color: "var(--color-ink-700)" }}>
  ...
  return (
    ...
    >
        ...
        {steps.map((step, i) => (
          <div key={step.label} className="flex items-center flex-1 last:flex-none">
            ...
            <div className="flex flex-col items-center gap-1 shrink-0">
              ...
              <span
                className="text-m-caption font-bold text-center leading-tight"
                style={{ color: step.state === "pending" ? "var(--color-ink-400)" : "var(--color-ink-950)" }}
              >
                {step.label}
              </span>
              {step.sub && step.state !== "pending" ? (
                <span className="text-m-caption text-center leading-tight" style={{ color: "var(--color-ink-500)" }}>
                  {step.sub}
                </span>
              ) : null}
              {step.docLabel && step.docUrl ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-m-caption font-semibold flex items-center gap-0.5 text-m-body press"
      ...
      {!saleDeedNo && !registryDocumentUrl && !isCompleted && (
        <p className="text-m-caption mt-2 pt-2" style={{ borderTop: "1px solid var(--color-line)", color: "var(--color-signal)" }}>
...
/* ═══════════════════════════════════════════════════════════
 * Edit sale modal — edits always-editable metadata
 * (notes, broker info, expected registry date, commission)
 * ═══════════════════════════════════════════════════════════ */
function EditSaleModal({
  ...
  initialBrokerName: string | null;
  initialBrokerPhone: string | null;
  initialBrokerAgency: string | null;
  initialCommissionAmount: number | null;
  initialExpectedRegistryDate: string | null;
  onClose: () => void;
  onSuccess: () => void;
}) {
  const [notes, setNotes] = useState(initialNotes ?? "");
  const [brokerName, setBrokerName] = useState(initialBrokerName ?? "");
  const [brokerPhone, setBrokerPhone] = useState(initialBrokerPhone ?? "");
  const [brokerAgency, setBrokerAgency] = useState(initialBrokerAgency ?? "");
  const [commissionAmount, setCommissionAmount] = useState(
    initialCommissionAmount != null ? String(initialCommissionAmount) : "",
  );
  const [expectedRegistryDate, setExpectedRegistryDate] = useState(
    initialExpectedRegistryDate ? initialExpectedRegistryDate.split("T")[0]! : "",
  );
    ...
    try {
      const res = await fetch(`/api/sales/${saleId}`, {
        ...
        body: JSON.stringify({
          ...
          expectedRegistryDate: expectedRegistryDate || null,
        }),
      });
      const data = await res.json();
