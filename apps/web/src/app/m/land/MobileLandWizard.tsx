...
export function MobileLandWizard({
  ...
  function setLandField<K extends keyof typeof land>(
    key: K,
    value: (typeof land)[K],
  ) {
  ...
  useEffect(() => {
    if (mode === "WHOLE" && sections.length === 1) {
      setSections((prev) =>
        prev.map((s, i) => (i === 0 ? { ...s, area: land.totalArea } : s)),
      );
    }
  ...
  const leaseRentAmount =
    isLeasehold && baseCostNum > 0 && Number(land.leaseRentPercent) > 0
      ? (baseCostNum * Number(land.leaseRentPercent)) / 100
      : 0;
  const gstAmount =
    isLeasehold && leaseRentAmount > 0 && Number(land.gstPercent) > 0
      ? (leaseRentAmount * Number(land.gstPercent)) / 100
      : 0;
  const registrationAmount =
    baseCostNum > 0 && Number(land.registrationPercent) > 0
      ? (baseCostNum * Number(land.registrationPercent)) / 100
      : 0;
  const stampDutyAmount =
    baseCostNum > 0 && Number(land.stampDutyPercent) > 0
      ? (baseCostNum * Number(land.stampDutyPercent)) / 100
      : 0;
  const transferDutyAmount =
    baseCostNum > 0 && Number(land.transferDutyPercent) > 0
      ? (baseCostNum * Number(land.transferDutyPercent)) / 100
      : 0;
  ...
  const totalLeaseRentOverTerm =
    isYearlyLease && leaseRentAmount > 0 && leasePeriodNum > 0
      ? leaseRentAmount * leasePeriodNum
      : 0;
  ...
  const sectionsAreaSum = sections.reduce(
    (s, sec) => s + (Number(sec.area) || 0),
    0,
  );
  ...
  function validateStep1(): boolean {
    ...
    if (baseCostNum <= 0) {
      toast.error("Base cost must be > 0");
      return false;
    }
  ...
  function validateStep2(): boolean {
    if (mode === "SUBDIVIDED" && sections.length < 2) {
      toast.error("Subdivided needs ≥2 sections");
      return false;
    }
    for (let i = 0; i < sections.length; i++) {
      const s = sections[i]!;
      if (!s.number.trim()) {
        toast.error(`Section ${i + 1}: number required`);
        return false;
      }
      if (Number(s.area) <= 0) {
        toast.error(`Section ${i + 1}: area must be > 0`);
        return false;
      }
      if (
        s.purpose === "PROJECT" &&
        !s.projectId &&
        !s.projectCreateName.trim()
      ) {
    ...
    if (mode === "SUBDIVIDED" && !areaValid) {
      toast.error(
        `Areas (${formatNumber(sectionsAreaSum, 2)}) must sum to total (${formatNumber(totalAreaNum, 2)})`,
      );
      return false;
    }
    return true;
  ...
  async function onSubmit() {
    ...
    try {
      const payload = {
        ...
        leasePeriodYears: isLeasehold
          ? land.leasePeriodYears
            ? Number(land.leasePeriodYears)
            : null
          : null,
        ...
        leaseRentPercent:
          isLeasehold && land.leaseRentPercent
            ? Number(land.leaseRentPercent)
            : null,
        leaseRentAmount:
          isLeasehold && leaseRentAmount ? leaseRentAmount : null,
        gstPercent:
          isLeasehold && land.gstPercent ? Number(land.gstPercent) : null,
        gstAmount: isLeasehold && gstAmount ? gstAmount : null,
        registrationPercent: land.registrationPercent
          ? Number(land.registrationPercent)
          : null,
        registrationAmount: registrationAmount || null,
        stampDutyPercent: land.stampDutyPercent
          ? Number(land.stampDutyPercent)
          : null,
        stampDutyAmount: stampDutyAmount || null,
        // Transfer duty (authority land — DDA, HUDCO, etc.)
        transferDutyPercent: land.transferDutyPercent
          ? Number(land.transferDutyPercent)
          : null,
          ...
          projectCreate:
            ...
            s.projectCreateName.trim()
              ? {
                  ...
                  address:
                    s.projectCreateAddress.trim() ||
                    land.location.trim() ||
                    null,
                  totalBudget: s.projectCreateBudget
                    ? Number(s.projectCreateBudget)
                    : undefined,
      ...
      toast.success("Land purchase recorded", {
        description:
          mode === "WHOLE"
            ? "1 parcel created."
            : `${data.parcelCount || sections.length} parcels created.`,
      });
          ...
          {["Details", "Sections", "Review", "Legal"].map((label, i) => (
            <div key={label} className="flex items-center gap-1 flex-1">
              <div className="flex items-center gap-1.5 flex-1">
                <div
                  className="grid place-items-center size-4 rounded-full text-[0.5rem] font-bold"
                  style={{
                    backgroundColor:
                      step === i + 1
                        ? "var(--color-ink-950)"
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setLandField("leaseType", "YEARLY");
                          haptic(10);
                        }}
                        className="rounded-[0.375rem] border py-2 text-center transition-colors"
                        style={{
                          borderColor:
                            land.leaseType === "YEARLY"
                              ? "var(--color-ink-950)"
                              : "var(--color-line)",
                          backgroundColor:
                            land.leaseType === "YEARLY"
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        min={0}
                        step="any"
                        value={land.leasePeriodYears}
                        onChange={(e) =>
                          setLandField("leasePeriodYears", e.target.value)
                        }
                        placeholder="99"
                        inputMode="numeric"
                        className={inputClass}
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={land.leaseStartDate}
                        onChange={(e) =>
                          setLandField("leaseStartDate", e.target.value)
                        }
                        className={inputClass}
                        style={inputStyle}
                      />
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={land.leaseEndDate}
                        onChange={(e) =>
                          setLandField("leaseEndDate", e.target.value)
                        }
                        className={inputClass}
                        style={inputStyle}
                      />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        min={0}
                        step="any"
                        value={land.leaseRentPercent}
                        onChange={(e) =>
                          setLandField("leaseRentPercent", e.target.value)
                        }
                        placeholder="10"
                        inputMode="decimal"
                        className={inputClass}
                    ...
                    <div>
                      ...
                      <div
                        className="h-10 flex items-center rounded-[0.5rem] border px-3 text-[0.6875rem]"
                        style={{
                          borderColor: "var(--color-line)",
                          backgroundColor: "var(--color-concrete)",
                          color: "var(--color-ink-500)",
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        min={0}
                        step="any"
                        value={land.gstPercent}
                        onChange={(e) =>
                          setLandField("gstPercent", e.target.value)
                        }
                        placeholder="18"
                        inputMode="decimal"
                        className={inputClass}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      min={0}
                      step="any"
                      value={land.registrationPercent}
                      onChange={(e) =>
                        setLandField("registrationPercent", e.target.value)
                      }
                      placeholder="1"
                      inputMode="decimal"
                      className={inputClass}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      min={0}
                      step="any"
                      value={land.stampDutyPercent}
                      onChange={(e) =>
                        setLandField("stampDutyPercent", e.target.value)
                      }
                      placeholder="5"
                      inputMode="decimal"
                      className={inputClass}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      min={0}
                      step="any"
                      value={land.transferDutyPercent}
                      onChange={(e) =>
                        setLandField("transferDutyPercent", e.target.value)
                      }
                      placeholder="0"
                      inputMode="decimal"
                      className={inputClass}
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      min={0}
                      step="0.01"
                      value={land.brokerageAmount}
                      onChange={(e) =>
                        setLandField("brokerageAmount", e.target.value)
                      }
                      placeholder="0"
                      inputMode="decimal"
                      className={inputClass}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min={0}
                      step="0.01"
                      value={land.legalFees}
                      onChange={(e) =>
                        setLandField("legalFees", e.target.value)
                      }
                      placeholder="0"
                      inputMode="decimal"
                      className={inputClass}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min={0}
                      step="0.01"
                      value={land.otherCharges}
                      onChange={(e) =>
                        setLandField("otherCharges", e.target.value)
                      }
                      placeholder="EDC/IDC"
                      inputMode="decimal"
                      className={inputClass}
            ...
            <>
              {mode === "WHOLE" ? (
                ...
                <>
                  ...
                  {/* Area conservation */}
                  <div
                    className="rounded-[0.5rem] px-3 py-2 text-[0.5625rem]"
                    style={{
                      border: `1px solid ${areaValid ? "var(--color-line)" : "var(--color-stop)"}`,
                      backgroundColor: areaValid
                        ? "var(--color-concrete)"
                        : "rgba(239,68,68,0.05)",
                      color: areaValid
                        ? "var(--color-ink-500)"
                  ...
                  >
                    Sections:{" "}
                    <strong>
                      {formatNumber(sectionsAreaSum, 2)} {land.areaUnit}
                    </strong>
                    {" / "}Total:{" "}
                    <strong>
                    ...
                    {!areaValid && (
                      <span> — Diff: {formatNumber(areaDiff, 2)}</span>
                    )}
          ...
          {step === 3 && (
            <>
              <div style={cardStyle} className="space-y-1.5">
                ...
                <div
                  className="pt-1.5 space-y-0.5 text-[0.5625rem]"
                  style={{ borderTop: "1px solid var(--color-line)" }}
                >
              ...
              <div
                className="text-[0.6875rem] font-bold pt-1"
                style={{ color: "var(--color-ink-950)" }}
              >
              ...
              <div className="space-y-2">
                {sections.map((s, i) => (
                  <div key={s.id} style={cardStyle} className="space-y-1">
                    ...
                    <div
                      className="grid grid-cols-3 gap-2 text-[0.5625rem]"
                      style={{ color: "var(--color-ink-500)" }}
                    >
                      <div>
                        Area:{" "}
              ...
              >
                ...
                <div>
                  ...
                  >
                    ...
                    also add these later from the Legal tab.
                  </div>
                </div>
              </div>
...
function MobileSectionEditor({
  ...
  onRemove: () => void;
  onProjectCreated: (p: ProjectOption) => void;
  inputClass: string;
  ...
  const cost =
    sectionsAreaSum > 0
      ? (totalCost * (Number(section.area) || 0)) / sectionsAreaSum
      : 0;
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[
            ...
            <button
              ...
              type="button"
              onClick={() => {
                onUpdate({ purpose: opt.value });
                haptic(10);
              }}
              className="rounded-[0.375rem] border py-2 text-center transition-colors"
              style={{
                borderColor:
                  section.purpose === opt.value
                    ? "var(--color-ink-950)"
                    : "var(--color-line)",
                backgroundColor:
                  section.purpose === opt.value
            ...
            >
              <opt.icon
                className="mx-auto size-3.5 mb-0.5"
                style={{
                  color:
                    section.purpose === opt.value
                      ? "var(--color-ink-950)"
...
function MobilePurposeBadge({ purpose }: { purpose: Purpose }) {
  ...
  return (
    <span
      className="rounded-full px-2 py-0.5 text-[0.5rem] font-bold"
