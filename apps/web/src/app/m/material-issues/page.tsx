...
export default function MobileMaterialIssuesPage() {
  return (
    <Suspense fallback={<MobileSkeletonList rows={6} />}>
      <MobileMaterialIssuesContent />
    </Suspense>
  );
}

async function MobileMaterialIssuesContent() {
  await connection();
  const company = await getCompany();
  const role = await getUserRole();
  const canView = hasPermission(role, PERM.INVENTORY_VIEW);
  ...
  if (!canView) {
    return (
      <MobileEmptyState
        icon={Package}
        title="No access"
        description="You need inventory view permission to see material issues."
      />
    );
  }
