 ...
 * action buttons, line items, totals, receipts, and payment history
 * without a modal overlay. Used inside the procurement SplitView.
 */
    ...
    <div className="space-y-4 p-4">
      ...
      <div className="flex items-start justify-between gap-3 border-b border-border pb-3">
        <div className="min-w-0">
          <h2 className="text-title font-bold text-foreground truncate">{po.poNumber}</h2>
          <p className="mt-0.5 text-caption text-muted-foreground">
            {po.supplierName} · {po.procurementScope === "COMPANY" ? "Company scope" : `Project: ${po.projectName ?? "—"}`}
          </p>
        </div>
        <a
