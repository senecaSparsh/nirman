...
export function DepartmentActivityFeed({ department }: { department: DepartmentKey }) {
  ...
  if (!expanded) {
    return (
      <button
        ...
        className="group mb-3 flex w-full items-center gap-2.5 border-0 bg-transparent py-1.5 text-left no-print"
      ...
      >
        ...
        <span className="flex items-center gap-1.5 shrink-0">
          ...
          <span className="text-micro font-semibold text-muted-foreground">LIVE</span>
        </span>
        ...
        {/* Count badge */}
        {rows.length > 0 && (
          <span className="shrink-0 text-micro font-medium tabular-nums text-faint">
            {rows.length}
          </span>
        )}

        <ChevronDown className="size-3.5 shrink-0 text-faint transition-transform group-hover:translate-y-0.5" />
      </button>
    );
  }
  ...
  // ── Expanded state ──
  return (
    <div className="mb-3 border-0 bg-transparent no-print">
      {/* Header bar */}
      <button
        onClick={() => setExpanded(false)}
        className="flex w-full items-center gap-2.5 border-0 bg-transparent py-1.5 text-left"
      ...
      >
        ...
        <ChevronDown className="size-3.5 shrink-0 rotate-180 text-faint transition-transform" />
      </button>

      {/* Feed list */}
      <div className="max-h-80 overflow-y-auto">
        {isLoading && !data ? (
          <div className="flex items-center gap-2.5 py-4 text-caption text-muted-foreground">
            <Circle className="size-1.5 fill-current text-faint animate-pulse" />
            Loading activity…
          </div>
        ) : rows.length === 0 ? (
          <div className="flex items-center gap-2.5 py-4 text-caption text-muted-foreground">
            <Circle className="size-1.5 fill-current text-faint" />
          ...
          <ul>
            {rows.map((row) => (
              ...
              >
                ...
                <span className="shrink-0 pt-px text-micro tabular-nums text-faint">
