 ...
 *
 * Uses the shared `.skeleton` gradient-sweep utility (same one as desktop
 * `PageLoading` / `SkeletonRows`) so mobile and desktop shimmer with one
 * consistent motion. Previously these used `animate-pulse` (opacity blink)
 * with no background — which meant the lines were invisible, since
 * `animate-pulse` only animates opacity and there was nothing to fade.
 *
...
export function SkeletonCard({ className }: { className?: string }) {
  return (
    <div className={cn("rounded-[0.625rem] border p-3", className)}>
  ...
  return (
    <div className="grid grid-cols-2 gap-2 p-3">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="rounded-[0.625rem] border p-2.5">
